...
SwaggerAuthorizations.prototype.setSecretKey = function(secretKey) {
  this.secretKey = secretKey
};

SwaggerAuthorizations.prototype.setApiKey = function(apiKey) {
  this.apiKey = apiKey
};
...
SwaggerAuthorizations.prototype.apply = function(obj, authorizations) {
  if(typeof this.secretKey != 'undefined' && typeof this.apiKey != 'undefined') {
    // Override obj.url with the signed url
    obj.url = this.getUrlWithSignature(obj, this.apiKey, this.secretKey);
  }
...
SwaggerAuthorizations.prototype.getUrlWithSignature = function(obj, apiKey, secretKey) {
  // Use a HTML <a> element to parse the url
  var urlObject = document.createElement('a');
  // Once href is set all other properties are parsed from the href
  urlObject.href = obj.url;

  var urlPath = urlObject.pathname
  // Remove trailing slash from url path
  urlPath = urlPath.replace(/\/$/, "");

  var urlQuery = urlObject.search;
  // Remove a leading '?' if it exists
  urlQuery = urlQuery.replace(/^\?/, "");
  // Replace any occurances of multiple ampersands with a single ampersand
  urlQuery = urlQuery.replace(/&&+/g, "&");
  // Remove a trailing ampersand if it exists
  urlQuery = urlQuery.replace(/&$/, "");

  // Add api key to url query
  urlQuery = urlQuery + "&api_key=" + apiKey;

  // Sort query parameters
  urlQuery = this.getSortedQueryParameters(urlQuery);

  // If the port number is a default port like 80 for http or 443 for https then we don't want to add the port.
  ...
  if(port == "80" || port == "443" || port == "0" || port == "") {
    // Found a default port. Dont append the port to the hostname
    hostPort = urlObject.hostname;
  } else {
    hostPort = urlObject.hostname + ":" + port;
  }
...
SwaggerAuthorizations.prototype.getSortedQueryParameters = function(urlQuery) {
  // Get all query parameter keys
  var urlQueryParameters = urlQuery.split("&");
  var urlQueryParameterNames = [];
  var urlQueryParameterNameValueMap = {};
  for(var i=0; i<urlQueryParameters.length; i++) {
    var nameValuePair = urlQueryParameters[i].split("=");
    var name = nameValuePair[0];
    var value = nameValuePair[1];

    // Add name to names list
    urlQueryParameterNames[i] = name;
    // Add to name value map
    urlQueryParameterNameValueMap[name] = value;
  }

  // Sort query parameter names
  urlQueryParameterNames.sort();

  // Construct sorted url query string
  var urlQuerySorted = "";
  var parameterSeparator = "";

  for(var i=0; i<urlQueryParameterNames.length; i++) {
    var parameterName = urlQueryParameterNames[i];
    var parameterValue = urlQueryParameterNameValueMap[parameterName];
    urlQuerySorted = urlQuerySorted + parameterSeparator + parameterName + "=" + parameterValue;
    parameterSeparator = "&";
  }

  return urlQuerySorted;
}
